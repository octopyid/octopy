...
// NuxtLink as component object: its name as a string leaves <component :is>
// unresolved and renders a dead custom element.
const tag = computed(() => (props.to ? NuxtLink : props.href ? 'a' : 'button'));
const isNativeButton = computed(() => tag.value === 'button');

const variantClasses: Record<ButtonVariant, string> = {
  primary: 'bg-primary-500 text-white shadow-sm hover:-translate-y-1 hover:bg-primary-600',
  secondary:
